...
const MIGRATION_KEY = "heide:indexeddb-uuid-migration-v1";
const BACKUP_KEY = "heide:indexeddb-backup";
...
async function backupDataBeforeMigration() {
  ...
  try {
    ...
    const db = await new Promise<IDBDatabase>((resolve, reject) => {
      const request = indexedDB.open("heideOS", 4); // Open version 4 explicitly
...
export async function migrateIndexedDBToUUIDs() {
  ...
  const currentDBVersion = await new Promise<number>((resolve) => {
    const request = indexedDB.open("heideOS");
    request.onsuccess = () => {
