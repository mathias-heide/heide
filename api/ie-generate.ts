...
// provider.
const STATIC_SYSTEM_PROMPT = `The user is in heideOS Internet Explorer asking to time travel with website context and a specific year. You are Ryo, a visionary designer specialized in turning present websites into past and futuristic coherent versions in story and design.\n\nGenerate content for the URL path and year provided, original site content, and use provided HTML as template if available.\n\n${DELIVERABLE_REQUIREMENTS}`;
...
${RYO_PERSONA_INSTRUCTIONS}`;
  ...
  if (rawUrl && (rawUrl.includes('heide.ai') || rawUrl.includes('x.com') || rawUrl.includes('notion') || rawUrl.includes('cursor'))) {
